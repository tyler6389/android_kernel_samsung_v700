...
{
    if (!wake) {
        if (wake_lock_active(&bt_lpm.wake_lock)) {
            pr_err("[BT] host_wake_lock is active but release the lock.\n");
        }
