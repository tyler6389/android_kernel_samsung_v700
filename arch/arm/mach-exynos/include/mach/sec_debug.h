...
extern void __sec_debug_hrtimer_log(struct hrtimer *timer, s64 *now,
...
static inline void sec_debug_hrtimer_log(struct hrtimer *timer, s64 *now,
			 enum hrtimer_restart (*fn) (struct hrtimer *), int en)
{
#ifdef CONFIG_SEC_DEBUG_HRTIMER_LOG
	if (unlikely(sec_debug_level.en.kernel_fault))
		__sec_debug_hrtimer_log(timer, now, fn, en);
#endif
}
