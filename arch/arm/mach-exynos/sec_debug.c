...
struct auxiliary_log {
	...
	struct auxiliary_info HrTimerLog[AUX_LOG_HRTIMER_MAX];
...
static atomic_t gExcpAuxHrTimerLogIdx = ATOMIC_INIT(-1);
...
void __sec_debug_hrtimer_log(struct hrtimer *timer, s64 *now,
		     enum hrtimer_restart (*fn) (struct hrtimer *), int en)
{
	...
	psec_debug_log->hrtimers[cpu][i].now = *now;
...
void sec_debug_aux_log(int idx, char *fmt, ...)
{
	...
	case SEC_DEBUG_AUXLOG_CPU_BUS_CLOCK_CHANGE:
		...
		strncpy((*gExcpAuxLogPtr).CpuClockLog[i].log,
			buf, AUX_LOG_LENGTH);
		break;
	case SEC_DEBUG_AUXLOG_HRTIMER_CHANGE:
		i = atomic_inc_return(&gExcpAuxHrTimerLogIdx)
			& (AUX_LOG_HRTIMER_MAX - 1);
		(*gExcpAuxLogPtr).HrTimerLog[i].time = cpu_clock(cpu);
		(*gExcpAuxLogPtr).HrTimerLog[i].cpu = cpu;
		strncpy((*gExcpAuxLogPtr).HrTimerLog[i].log,
			buf, AUX_LOG_LENGTH);
		break;
