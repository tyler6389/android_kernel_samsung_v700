...
static void tsu8111_test_read_regs(struct i2c_client *client)
{
	u8 data = 0;
	u32 addr = 0;
	...
	for (addr = 0x01; addr <= 0x0d; addr++) {
		tsu8111_i2c_read(client, addr, &data);
		dev_info(&client->dev, "%s : 0x%02x (0x%02x)\n",
			__func__, addr, data);
	}

	tsu8111_i2c_read(client, TSU8111_MANUALSW1, &data);
	dev_info(&client->dev, "%s : 0x%02x (0x%02x)\n",
		__func__, TSU8111_MANUALSW1, data);
	tsu8111_i2c_read(client, TSU8111_MANUALSW2, &data);
	dev_info(&client->dev, "%s : 0x%02x (0x%02x)\n",
		__func__, TSU8111_MANUALSW2, data);
	tsu8111_i2c_read(client, TSU8111_DEVICE_TYPE3, &data);
	dev_info(&client->dev, "%s : 0x%02x (0x%02x)\n",
		__func__, TSU8111_DEVICE_TYPE3, data);
	tsu8111_i2c_read(client, TSU8111_RESET, &data);
	dev_info(&client->dev, "%s : 0x%02x (0x%02x)\n",
		__func__, TSU8111_RESET, data);
...
static int tsu8111_get_cable_type(
		struct i2c_client *client)
{
	...
	tsu8111_i2c_read(client, TSU8111_INTERRUPT1, &data1);
	tsu8111_i2c_read(client, TSU8111_INTERRUPT2, &data2);

	tsu8111_i2c_read(client, TSU8111_DEVICE_TYPE1, &data1);
	tsu8111_i2c_read(client, TSU8111_DEVICE_TYPE2, &data2);

	tsu8111_test_read_regs(client);

	if (data1 & 0x02) {	/* VBUS valid */
		if (data1 & 0x04)	/* USB */
			cable_type = POWER_SUPPLY_TYPE_USB;
		else if (data1 & 0x70)	/* DCP, CDP, Type1/Type2 Charger */
	...
	if (charger->pdata->cable_source_type &
		...
		cable_type <<= ONLINE_TYPE_MAIN_SHIFT;

	dev_dbg(&client->dev, "%s : device1(0x%02x), device2(0x%02x)\n",
		__func__, data1, data2);
	dev_dbg(&client->dev, "%s : cable_type(%d)\n", __func__, cable_type);
	return cable_type;
}
			      ...
			      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		val->intval = tsu8111_get_cable_type(client);
		break;
