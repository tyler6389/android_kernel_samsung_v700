...
int fimc_is_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		dev_lock(is_dev->bus_dev, dev, BUS_LOCK_FREQ_L4);
#else
		...
		is_dev->af.mode = IS_FOCUS_MODE_AUTO;
		is_dev->af.af_lock_state = FIMC_IS_AF_UNLOCKED;
		is_dev->af.ae_lock_state = FIMC_IS_AE_UNLOCKED;
		is_dev->af.awb_lock_state = FIMC_IS_AWB_UNLOCKED;
...
#define FRONT_CAM_STANDARD_REVISION	0x0b
	...
	case V4L2_CID_IS_MIPI_SETTLE:
		ctrl->value = dev->mipi_settle;
		printk(KERN_INFO "g_ctrl Settle line= %d\n", ctrl->value);
		break;
#endif
	default:
		return -EINVAL;
...
static int fimc_is_v4l2_af_mode(struct fimc_is_dev *dev, int value)
{
	...
	case FOCUS_MODE_AUTO:
		...
		IS_ISP_SET_PARAM_AA_MANUAL_AF(dev, 0);  //infinity
		IS_SET_PARAM_BIT(dev, PARAM_ISP_AA);
		IS_INC_PARAM_NUM(dev);
		dev->af.af_state = FIMC_IS_AF_SETCONFIG;
	...
	case FOCUS_MODE_MACRO:
		...
		IS_ISP_SET_PARAM_AA_TOUCH_X(dev, 0);
		IS_ISP_SET_PARAM_AA_TOUCH_Y(dev, 0);
		IS_ISP_SET_PARAM_AA_MANUAL_AF(dev, 600);
		IS_SET_PARAM_BIT(dev, PARAM_ISP_AA);
		IS_INC_PARAM_NUM(dev);
		dev->af.af_state = FIMC_IS_AF_SETCONFIG;
		dev->af.af_lock_state = 0;
	...
	case FOCUS_MODE_FIXED:
		...
		IS_ISP_SET_PARAM_AA_TOUCH_X(dev, 0);
		IS_ISP_SET_PARAM_AA_TOUCH_Y(dev, 0);
		IS_ISP_SET_PARAM_AA_MANUAL_AF(dev, 250);
		IS_SET_PARAM_BIT(dev, PARAM_ISP_AA);
		IS_INC_PARAM_NUM(dev);
		dev->af.af_state = FIMC_IS_AF_SETCONFIG;
		dev->af.af_lock_state = 0;
...
static int fimc_is_v4l2_af_start_stop(struct fimc_is_dev *dev, int value)
{
	...
	printk("\nfimc_is_v4l2_af_start_stop : ( %d) (mode:%d)\n",value,dev->af.mode);
	...
	case AUTO_FOCUS_CANCEL:
		if (!is_af_use(dev)) {
			...
			case IS_FOCUS_MODE_MACRO:
				...
				break;
			default:
				/* If other AF mode, there is no cancelation process*/
				break;
		}
        }
        break;
...
void fimc_is_frame_delay(struct fimc_is_dev *dev, int frame_num)
{
	...
	printk("fimc_is_frame_delay : start frame : %d \n", dev->frame_count);
	curr_frame_count = dev->frame_count + frame_num;
	wait_event_timeout(dev->irq_queue1,
		(dev->frame_count >= curr_frame_count), 25*HZ/100);  /* 250ms, Minimum 4fps */
	printk("end frame: %d\n", dev->frame_count);
...
{
	int ret = 0;
	printk("fimc_is_v4l2_ae_awb_lockunlock : value( %d )\n", value);

	switch (value) {
	case AE_UNLOCK_AWB_UNLOCK:
		if(dev->af.ae_lock_state == FIMC_IS_AE_LOCKED||
		    dev->af.awb_lock_state == FIMC_IS_AWB_LOCKED){

			dev->af.ae_lock_state = FIMC_IS_AE_SETCONFIG;
			dev->af.awb_lock_state = FIMC_IS_AWB_SETCONFIG;

			IS_ISP_SET_PARAM_AA_CMD(dev, ISP_AA_COMMAND_START);
			IS_ISP_SET_PARAM_AA_TARGET(dev, ISP_AA_TARGET_AE |ISP_AA_TARGET_AWB);
			IS_SET_PARAM_BIT(dev, PARAM_ISP_AA);
			IS_INC_PARAM_NUM(dev);
			fimc_is_mem_cache_clean((void *)dev->is_p_region, IS_PARAM_SIZE);
			fimc_is_hw_set_param(dev);
	...
	case AE_LOCK_AWB_UNLOCK:
		if(dev->af.ae_lock_state == FIMC_IS_AE_UNLOCKED){
		...
		if(dev->af.awb_lock_state == FIMC_IS_AWB_LOCKED){
	...
	case AE_UNLOCK_AWB_LOCK:
		...
		if(dev->af.awb_lock_state == FIMC_IS_AWB_UNLOCKED){
	...
	case AE_LOCK_AWB_LOCK:
		if(dev->af.ae_lock_state == FIMC_IS_AE_UNLOCKED ||
			...
			printk("AE/AWB was already locked: %s\n", __func__);
		}
		break;
	default:
		break;
...
static int fimc_is_v4l2_mode_change(struct fimc_is_dev *dev, int value)
{
	...
	/* auto focus should be locked before changing scenario mode */
	fimc_is_v4l2_af_start_stop(dev, AUTO_FOCUS_OFF);
...
#ifdef CONFIG_VIDEO_EXYNOS_FIMC_IS_BINNING_2X2
static int fimc_is_v4l2_sensor_mode_change(struct fimc_is_dev *is_dev, int value)
{
	int ret = 0;
	int sensor_mode = 0;
	...
	clear_bit(IS_ST_CHANGE_SENSOR_MODE, &is_dev->state);

	switch (value) {
	case IS_MODE_FULL:
		sensor_mode = SENSOR_NAME_S5K6A3;
		is_dev->sensor.binning_ratio_x = 1000; // doesn't use binning
		is_dev->sensor.binning_ratio_y = 1000; // doesn't use binning
		break;
	case IS_MODE_BINNING_2X2:
		sensor_mode = SENSOR_NAME_S5K6A3_BINNING_2X2;
		is_dev->sensor.binning_ratio_x = 2000; //use 1/2 sensor binning
		is_dev->sensor.binning_ratio_y = 2000;	//use 1/2 sensor binning
		break;
	}

	printk("fimc_is_v4l2_sensor_mode_change E (%d) +++++\n", value);

	fimc_is_hw_wait_intmsr0_intmsd0(is_dev);
	writel(HIC_SENSOR_MODE_CHANGE, is_dev->regs + ISSR0);
	writel(sensor_mode, is_dev->regs + ISSR2);
	fimc_is_hw_set_intgr0_gd0(is_dev);

	ret = wait_event_timeout(is_dev->irq_queue1,
			test_bit(IS_ST_CHANGE_SENSOR_MODE, &is_dev->state),
						FIMC_IS_SHUTDOWN_TIMEOUT);
	if (!ret) {
		err("Sendor Mode change timeout !!\n");

		fimc_is_hw_set_low_poweroff(is_dev, true);
		return -EINVAL;
	}

	printk("fimc_is_v4l2_sensor_mode_change X -----\n");
	return ret;
}
#endif
...
static int fimc_is_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch (ctrl->id) {
#ifdef CONFIG_VIDEO_EXYNOS_FIMC_IS_BINNING_2X2
	case V4L2_CID_IS_S_MODE_CHANGE:
		ret = fimc_is_v4l2_sensor_mode_change(dev, ctrl->value);
		break;
#endif
	...
	struct v4l2_mbus_framefmt *mf)
{
	...
	/* 1. ISP input / output*/
#ifdef CONFIG_VIDEO_EXYNOS_FIMC_IS_BINNING_2X2
	current_sensor_mode = readl(dev->regs + ISSR2);

	if (current_sensor_mode == SENSOR_NAME_S5K6A3_BINNING_2X2) {
		dev->sensor.binning_ratio_x = 2000; //use 1/2 sensor binning
		dev->sensor.binning_ratio_y = 2000;	//use 1/2 sensor binning
	} else if (current_sensor_mode == SENSOR_NAME_S5K6A3) {
		dev->sensor.binning_ratio_x = 1000; // doesn't use binning
		dev->sensor.binning_ratio_y = 1000; // doesn't use binning
	} else {
		printk("Warning!! sensor_mode is invalid (%d)\n", current_sensor_mode);
	}
	IS_ISP_SET_PARAM_OTF_INPUT_BINNING_RATIO_X(dev, dev->sensor.binning_ratio_x);
	IS_ISP_SET_PARAM_OTF_INPUT_BINNING_RATIO_Y(dev, dev->sensor.binning_ratio_y);
#endif
	IS_ISP_SET_PARAM_OTF_INPUT_WIDTH(dev, mf->width);
	IS_ISP_SET_PARAM_OTF_INPUT_HEIGHT(dev, mf->height);
	IS_ISP_SET_PARAM_OTF_INPUT_FORMAT(dev, format);
