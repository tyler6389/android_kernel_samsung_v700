...
#define IS_REGION_VER 137  /* IS REGION VERSION 1.23 */
#else
...
#ifdef CONFIG_VIDEO_EXYNOS_FIMC_IS_BINNING_2X2
#define IS_ISP_SET_PARAM_OTF_INPUT_BINNING_RATIO_X(dev, x) \
		(dev->is_p_region->parameter.isp.otf_input.binning_ratio_x = x)
#define IS_ISP_SET_PARAM_OTF_INPUT_BINNING_RATIO_Y(dev, x) \
		(dev->is_p_region->parameter.isp.otf_input.binning_ratio_y = x)
#endif
...
#define IS_ISP_SET_PARAM_AA_AF_REGION(dev, x) \
		(dev->is_p_region->parameter.isp.aa.use_af_region = x)
#define IS_ISP_SET_PARAM_AA_AF_REGION_L(dev, x) \
		(dev->is_p_region->parameter.isp.aa.af_region_left = x)
#define IS_ISP_SET_PARAM_AA_AF_REGION_T(dev, x) \
		(dev->is_p_region->parameter.isp.aa.af_region_top = x)
#define IS_ISP_SET_PARAM_AA_AF_REGION_R(dev, x) \
		(dev->is_p_region->parameter.isp.aa.af_region_right = x)
#define IS_ISP_SET_PARAM_AA_AF_REGION_B(dev, x) \
		(dev->is_p_region->parameter.isp.aa.af_region_bottom = x)
...
struct param_otf_input {
	...
	u32  binning_ratio_x;
	u32  binning_ratio_y;
	u32	reserved[PARAMETER_MAX_MEMBER-15];
#else
...
struct param_isp_aa {
	u32	cmd;		// Start, Stop (abort)
	u32	target;		// AF | AE | AWB
	u32	mode;		//  Manual, Single, Continuous, Sleep + Init, Set Center/Touch/Face window
	u32	scene;		// Normal, Macro
	...
	u32	touch_x;	// Touched point X, Available only if (uiAfMode = Single || Continuous) && (uiAfTouch = Enable)
	u32	touch_y;	// Touched point Y, Available only if (uiAfMode = Single || Continuous) && (uiAfTouch = Enable)
	u32	manual_af_setting;	// 0(Infinity, Bottom) ~ 255(Macro, Top), Available only if (uiAfMode = Manual)
       u32  use_af_region;
	u32  af_region_left;
	u32  af_region_top;
	u32  af_region_right;
...
union sensor_peri_format
{
    struct _i2c {
        u32 channel;
        u32 slave_addr;      // device address(used at ispfw, Don't care at host side)
        u32 speed;                  // device speed(used at ispfw, Don't care at host side)
    } i2c;
    ...
    struct _gpio {
        u32 first_gpio_port_no;            // Gpio port num : 0~15
        u32 second_gpio_port_no;            // Gpio port num : 0~15
    } gpio;
};
...
struct sensor_protocol
{
	u32 name;
	u32 type;
	union sensor_peri_format setting;
};

struct sensor_open_extended {
	struct sensor_protocol actuator_type;
	struct sensor_protocol flash_type;
	struct sensor_protocol fRom_type;
	u32 mclk;
	u32 mipi_lane_num;
	u32 mipi_speed;
