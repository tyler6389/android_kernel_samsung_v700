...
static int dsim_activate_req(struct dsim_global *dsim, int force, int keepon);
static int dsim_deactivate_req(struct dsim_global *dsim, int force, int keepon, int req);
#endif

#ifdef CONFIG_EXYNOS_DEV_PD
#ifdef CONFIG_FB_I80_POWER_GATING
void s3cfb_runtime_get_sync(void);
void s3cfb_runtime_put_sync(void);
#endif
#endif

#ifdef CONFIG_FB_S5P_DSIM_ULPS
#define DSIM_REG_ESC_EXIT_ULPS_CLK_SHIFT	0x00
#define DSIM_REG_ESC_REQ_ULPS_CLK_SHIFT		0x01
#define DSIM_REG_ESC_EXIT_ULPS_DATA_SHIFT	0x02
#define DSIM_REG_ESC_REQ_ULPS_DATA_SHIFT	0x03

#define DSIM_REG_STATUS_ULPS_CLK_SHIFT		0x09
#define DSIM_REG_STATUS_ULPS_DATA0_SHIFT	0x04

void s5p_dsim_ulps_req_clk(unsigned int dsim_base)
{
	unsigned int reg;
	
	reg = readl(dsim_base + S5P_DSIM_ESCMODE);
	reg |= (0x01 << DSIM_REG_ESC_REQ_ULPS_CLK_SHIFT);

	writel(reg, dsim_base + S5P_DSIM_ESCMODE);
}

unsigned int s5p_dsim_check_ulps_clk(unsigned int dsim_base)
{
	unsigned int reg;
	
	reg = readl(dsim_base + S5P_DSIM_STATUS);
	reg &= (0x01 << DSIM_REG_STATUS_ULPS_CLK_SHIFT);

	return reg >> DSIM_REG_STATUS_ULPS_CLK_SHIFT;
}

void s5p_dsim_ulps_req_data(unsigned int dsim_base)
{
	unsigned int reg;
	
	reg = readl(dsim_base + S5P_DSIM_ESCMODE);
	reg |= (0x01 << DSIM_REG_ESC_REQ_ULPS_DATA_SHIFT);

	writel(reg, dsim_base + S5P_DSIM_ESCMODE);
}

unsigned int s5p_dsim_check_ulps_data(unsigned int dsim_base)
{
	unsigned int reg;

	reg = readl(dsim_base + S5P_DSIM_STATUS);
	reg &= (0x01 << DSIM_REG_STATUS_ULPS_DATA0_SHIFT);

	return reg >> DSIM_REG_STATUS_ULPS_DATA0_SHIFT;
}


void s5p_dsim_exit_ulps_clk(unsigned int dsim_base)
{
	unsigned int reg;
	
	reg = readl(dsim_base + S5P_DSIM_ESCMODE);
	reg |= (0x01 << DSIM_REG_ESC_EXIT_ULPS_CLK_SHIFT);

	writel(reg, dsim_base + S5P_DSIM_ESCMODE);

}

void s5p_dsim_exit_ulps_data(unsigned int dsim_base)
{
	unsigned int reg;
	
	reg = readl(dsim_base + S5P_DSIM_ESCMODE);
	reg |= (0x01 << DSIM_REG_ESC_EXIT_ULPS_DATA_SHIFT);

	writel(reg, dsim_base + S5P_DSIM_ESCMODE);
}

void s5p_dsim_clear_ulpl(unsigned int dsim_base)
{
	unsigned int reg;
...
int s5p_dsim_enter_ulps_mode(int sleep_context)
{
	int ret = 0;
	int retry_cnt = 10;
	unsigned int reg;
	...
	dev_info(dsim->dev, "%s was called\n", __func__);

	if (dsim->ulps_cont != DSIM_ULPS_EN) {
		s5p_dsim_ulps_req_clk(dsim->reg_base);
check_clk:
		reg = s5p_dsim_check_ulps_clk(dsim->reg_base);
		//dev_info(dsim->dev, "%s clk status : %x\n", __func__, reg);
		if (!reg) {
			if (retry_cnt--) {
				if (sleep_context == 1)
					usleep_range(1, 1);
				...
				dev_err(dsim->dev, "%s failed to enter ulps clk\n", __func__);
				return ret;
			}	
		}
		retry_cnt = 10;
		s5p_dsim_ulps_req_data(dsim->reg_base);
check_data:
		reg = s5p_dsim_check_ulps_data(dsim->reg_base);
		//dev_info(dsim->dev, "%s data status : %x\n", __func__, reg);
		if (!reg) {
			if (retry_cnt--) {
				if (sleep_context == 1)
					usleep_range(1, 1);
				...
				dev_err(dsim->dev, "%s failed to enter ulps data\n", __func__);
				return ret;
			}	
		}
		
		dsim->ulps_cont = DSIM_ULPS_EN;
	}

	return ret;
}

int s5p_dsim_exit_ulps_mode(int sleep_context)
{
	int ret = 0;
	int retry_cnt = 10;
	unsigned int reg;
	...
	if (dsim->ulps_cont != DSIM_ULPS_DIS) {
		s5p_dsim_exit_ulps_clk(dsim->reg_base);
check_clk:
		reg = s5p_dsim_check_ulps_clk(dsim->reg_base);
		//dev_info(dsim->dev, "%s data status : %x\n", __func__, reg);
		if (reg) {
			if (retry_cnt--) {
				...
				dev_err(dsim->dev, "%s failed to exit ulps clk\n", __func__);
				return ret;
			}	
		}
		retry_cnt = 10;
		s5p_dsim_exit_ulps_data(dsim->reg_base);
check_data:
		reg = s5p_dsim_check_ulps_data(dsim->reg_base);
		//dev_info(dsim->dev, "%s data status : %x\n", __func__, reg);
		if (reg) {
			if (retry_cnt--) {
				...
				dev_err(dsim->dev, "%s failed to exit ulps data\n", __func__);
				return ret;
			}	
		}
		s5p_dsim_clear_ulpl(dsim->reg_base);

		dsim->ulps_cont = DSIM_ULPS_DIS;
	}
...
int s5p_dsim_set_hs_mode(int force, int sleep_context)
{
	int ret = 0;
	int retry = 5;
	int retry_cnt = 1000;
...
enable_hs_mode:	
	if (dsim->non_cont != DSIM_HS_MODE) {
		s5p_dsim_enable_hs_clock(dsim->reg_base, DSIM_HS_MODE);
	
check_clk_status:
		clk_status = s5p_dsim_get_stop_state_clock(dsim->reg_base);
		if (clk_status) {
			if (retry_cnt--) {
				if (sleep_context == 1)
					usleep_range(1, 1);
					...
					udelay(1);
				goto check_clk_status;
			}
			else {
				if (retry--) {
					dev_err(dsim->dev, "failed to set hs mode retry : %d\n", retry);
					goto enable_hs_mode;
				}
				dev_err(dsim->dev, "failed to set hs mode\n");
				return ret;
			}	
		}
	}
	dsim->non_cont = DSIM_HS_MODE;

	return ret;
}
EXPORT_SYMBOL(s5p_dsim_set_hs_mode);

int s5p_dsim_set_lp_mode(int force, int sleep_context)
{
	int ret = 0;
	int retry = 5;
	int retry_cnt = 1000;
...
enable_lp_mode:
	if (dsim->non_cont != DSIM_LP_MODE) {
		s5p_dsim_enable_hs_clock(dsim->reg_base, DSIM_LP_MODE);
...
check_clk_status:
		...
		if (!clk_status) {
			if (retry_cnt--) {
				...
				if (retry--) {
					dev_err(dsim->dev, "failed to set lp mode retry : %d\n", retry);
					goto enable_lp_mode;
				}
				dev_err(dsim->dev, "failed to set lp mode\n");
				return ret;
			}	
		}
	}
	dsim->non_cont = DSIM_LP_MODE;
	return ret;
}
EXPORT_SYMBOL(s5p_dsim_set_lp_mode);
...
void s5p_dsim_req_frame_done(void)
{
	struct dsim_global *dsim = g_dsim;
...
#ifdef CONFIG_DSIM_NON_CONTINUOUS
	s5p_dsim_set_hs_mode(0, 0);
#endif
	
	if (dsim->frame_req) {
		dev_info(dsim->dev, "frame req value is already set to %d\n", dsim->frame_req);
		return;
	}
	dsim->frame_req++;
...
EXPORT_SYMBOL(s5p_dsim_req_frame_done);
#endif
...
#ifdef CONFIG_DSIM_ASYNC_WRITE
extern int s3cfb_get_draw_req(void);
#define DSIM_ASYNC_WRITE_TIMEOUT	msecs_to_jiffies(166)
#endif
void s5p_dsim_clear_frame_req(void)
{
	struct dsim_global *dsim = g_dsim;
...
EXPORT_SYMBOL(s5p_dsim_clear_frame_req);
	...
	unsigned int data_id, unsigned int data0, unsigned int data1)
{
...
#ifdef CONFIG_FB_I80_CLOCK_GATING
	dsim_activate_req(dsim, 0, 1);
#endif

#ifdef CONFIG_DSIM_NON_CONTINUOUS
	if (!dsim->non_cont)
		s5p_dsim_set_hs_mode(0, 1);
#endif

#ifdef CONFIG_DSIM_ASYNC_WRITE
	if (s3cfb_get_draw_req() || dsim->frame_req) {
		init_completion(&dsim->async_complete);
		if (!wait_for_completion_interruptible_timeout(&dsim->async_complete, DSIM_ASYNC_WRITE_TIMEOUT)) {
			dev_err(dsim->dev, "%s timeout\n", __func__);
		}
	}
#endif
	...
	case SET_MAX_RTN_PKT_SIZE:
		...
		ret = DSIM_TRUE; /* response should be implemented */
		goto exit;
	/* general command */
	case CMD_OFF:
	case CMD_ON:
	...
	case DCS_RD_NO_PARA:
		...
		s5p_dsim_wr_tx_header(dsim_base, (unsigned char) data_id,
			(unsigned char) data0, (unsigned char) data1);
		//mutex_unlock(&dsim_rd_wr_mutex);
		ret = DSIM_FALSE; /* response should be implemented. */
		goto exit;
	...
	case DCS_LONG_WR:
	{
		...
		/* below codes are valid only video mode. minwoo7945.kim*/
#ifndef CONFIG_FB_S5P_I80_LCD
...
exit:
#ifdef CONFIG_FB_I80_CLOCK_GATING
	dsim_deactivate_req(dsim, 0, 1, GATE_CLK_REQ);
#endif
	mutex_unlock(&dsim_rd_wr_mutex);
	return ret;
}
	...
	for (i = 0; i < 32; i++) {
		if (intmsk & (0x01<<i)) {
			...
			case S5P_DSIM_INT_MSK_FRAME_DONE:
				...
				if (dsim->frame_req) {
					dsim->frame_req--;
					if (!completion_done(&dsim->async_complete)) {
						complete_all(&dsim->async_complete);
					}
#ifdef CONFIG_DSIM_NON_CONTINUOUS 
					else {
						s5p_dsim_set_lp_mode(0, 0);
					}
#endif
				}
#else
...
void s5p_dsim_early_suspend(void)
{
	...
	if (dsim->mipi_ddi_pd->resume_complete == 0)
		return;
#ifdef CONFIG_FB_I80_CLOCK_GATING
	dsim->gate_flag = DSIM_GATE_DISABLE;
	dsim_activate_req(dsim, 1, 0);
#endif
...
#ifdef CONFIG_DSIM_ASYNC_WRITE
	if (dsim->frame_req) {
		dev_info(dsim->dev, "frame req is not zero\n");
		dsim->frame_req = 0;
	}
#endif
...
#ifdef CONFIG_FB_I80_CLOCK_GATING
	dsim_deactivate_req(dsim, 1, 0, GATE_CLK_REQ);
#else
...
static int dsim_activate_clk(struct dsim_global *dsim)
{
	int ret = 0;

	if (dsim->clk_stat == DSIM_CLK_ON)
		return ret;
	...
	if (dsim->reg_base) {
		writel(0xffffffff, dsim->reg_base + S5P_DSIM_INTSRC);
#ifdef CONFIG_FB_S5P_DSIM_ULPS
		s5p_dsim_exit_ulps_mode(1);
#endif
#ifdef CONFIG_DSIM_NON_CONTINUOUS
		if (dsim->non_cont != DSIM_HS_MODE)
			s5p_dsim_set_hs_mode(0, 1);
	...
	dsim->clk_stat = DSIM_CLK_ON;
	return ret;
}

static int dsim_deactivate_clk(struct dsim_global *dsim)
{
	int ret = 0;

	if (dsim->clk_stat == DSIM_CLK_OFF)
		return ret;

#ifdef CONFIG_DSIM_NON_CONTINUOUS
		if (dsim->non_cont != DSIM_LP_MODE)
			s5p_dsim_set_lp_mode(0, 1);
#endif

#ifdef CONFIG_FB_S5P_DSIM_ULPS
		s5p_dsim_enter_ulps_mode(1);
#endif

	if (dsim->clock->usage) {
		printk("dsim clk off\n");
		clk_disable(dsim->clock);
		dsim->clk_stat = DSIM_CLK_OFF;
	}
...
#ifdef CONFIG_FB_I80_POWER_GATING
static int dsim_activate_pwr(struct dsim_global *dsim)
{
	int ret = 0;
...
static int dsim_deactivate_pwr(struct dsim_global *dsim)
{
	int ret = 0;
	u32 int_stat = 0;
	pm_message_t state;

	if (dsim->clk_stat == DSIM_PWR_OFF)
		return ret;

	printk("dsim power off\n");
...
static int dsim_activate_req(struct dsim_global *dsim, int force, int keepon)
{
	...
	mutex_lock(&dsim->gate_mutex);	

	if (force)
		goto enable_clk;

	if (dsim->gate_flag != DSIM_GATE_ENABLE)
		goto exit;
...
enable_clk:
	if (dsim->clk_stat == DSIM_CLK_OFF)
		dsim_activate_clk(dsim);
#ifdef CONFIG_FB_I80_POWER_GATING
	else if (dsim->clk_stat == DSIM_PWR_OFF)
		dsim_activate_pwr(dsim);
#endif
exit:
	mutex_unlock(&dsim->gate_mutex);
	return ret;
}
...
static int dsim_deactivate_req(struct dsim_global *dsim, int force, int keepon, int req)
{
	...
	if (dsim->dsim_act_req == DSIM_ACTIVATE_REQ)
		goto exit;

deactivate_req:
	if (req == GATE_CLK_REQ)
		ret = dsim_deactivate_clk(dsim);
#ifdef CONFIG_FB_I80_POWER_GATING
	else if (req == GATE_PWR_REQ)
		ret = dsim_deactivate_pwr(dsim);
#endif
exit:
	mutex_unlock(&dsim->gate_mutex);	
	return ret;
...
int s5p_dsim_activate(int req_type)
{
	int ret = 0;
	...
	g_dsim->dsim_act_req = DSIM_ACTIVATE_REQ;
	g_dsim->gate_flag = DSIM_GATE_ENABLE;
	
	ret = dsim_activate_req(g_dsim, 0, 0);

	return ret;
}
EXPORT_SYMBOL(s5p_dsim_activate);

int s5p_dsim_deactivate(int req_type)
{
	int ret = 0;
	...
	g_dsim->dsim_act_req = DSIM_DEACTIVATE_REQ;
	
	ret = dsim_deactivate_req(g_dsim, 0, 0, req_type);

	return ret;
}
EXPORT_SYMBOL(s5p_dsim_deactivate);
...
err_clk_disable:
#ifdef CONFIG_FB_I80_CLOCK_GATING
	dsim_deactivate_req(dsim , 1, 1, GATE_CLK_REQ);
#else
	clk_disable(dsim->clock);
#endif
err_lcd_power_off:
...
static void s5p_dsim_shutdown(struct platform_device *pdev)
{
	...
	if (dsim->mipi_drv->shutdown) 
		dsim->mipi_drv->shutdown(&dsim->panel);
