...
int s3cfb_vsync_status_check(void)
{
	...
	if (fbdev[0]->system_state == POWER_ON)
#else
...
EXPORT_SYMBOL(s3cfb_runtime_get_sync);
...
EXPORT_SYMBOL(s3cfb_runtime_put_sync);
#endif
#endif
...
static void s3cfb_activate_vsync(struct s3cfb_global *fbdev)
{
	...
	if (!prev_refcount && fbdev->regs) {
		...
		s3cfb_activate_req(fbdev, 0);
#endif
		fbdev->vsync_cnt = 0;
		if (fbdev->lcd->enable_te)
			fbdev->lcd->enable_te();
		enable_irq(fbdev->lcd->te_irq);
#else
...
#endif
#ifdef CONFIG_FB_S5P_I80_LCD
		if (fbdev->lcd->disable_te)
			fbdev->lcd->disable_te();
		disable_irq(fbdev->lcd->te_irq);
#ifdef CONFIG_FB_I80_CLOCK_GATING
		s3cfb_deactivate_req(fbdev, 0, GATE_CLK_REQ);
#endif
...
int s3cfb_enable_ove_tune(int enable) 
{
	int i;
	struct s3cfb_lcd *lcd;
	struct s3cfb_global *ctrl;
	 
	ctrl = get_fimd_global(0);
	if (!ctrl)
		return 0;

	if (!ctrl->regs) {
		ctrl->ove_tune = enable;
		return 0;
	}
	lcd = ctrl->lcd;
	...
	if (enable) {
		if (lcd->reg_gamma) {
			for (i = 0; i < FIMD_GAMMA_REG_CNT; i++)
		 		writel(lcd->reg_gamma[i], ctrl->regs + (S3C_GAMMALUT_START + (i<<2)));
		}
		if (lcd->reg_gain)
			writel(lcd->reg_gain, ctrl->regs + S3C_CGAINCON);

		writel(lcd->reg_vidcon3, ctrl->regs + S3C_VIDCON3);
		ctrl->ove_tune = 1;
	} else {
		writel(0, ctrl->regs + S3C_VIDCON3);
		ctrl->ove_tune = 0;
	}
#if 0
	if (!ctrl->draw_req)
...
EXPORT_SYMBOL(s3cfb_enable_ove_tune);
#endif
...
extern void s5p_dsim_req_frame_done(void);

int s3cfb_get_draw_req(void)
{
	struct s3cfb_global *fbdev;
	fbdev = get_fimd_global(0);

	if (fbdev->system_state != POWER_ON)
		return 0;
	
	return fbdev->draw_req;
}
EXPORT_SYMBOL(s3cfb_get_draw_req);

void s3cfb_clear_draw_req(void)
{
...
EXPORT_SYMBOL(s3cfb_clear_draw_req);
#endif
#ifdef CONFIG_FB_S5P_I80_LCD
extern int get_panel_status(void);

#define FIMD_FPS_30
static irqreturn_t s3cfb_irq_te(int irq, void *dev_id)
{
	u32 cfg, update;
	struct s3cfb_global *fbdev;

	if (fbfimd->fbdev[0]->system_state == POWER_ON) {
		fbdev = get_fimd_global(0);
		if (fbdev->draw_req && get_panel_status()){
#ifdef CONFIG_DSIM_ASYNC_WRITE
			s5p_dsim_req_frame_done();
#endif
			cfg = readl(fbdev->regs + S3C_TRIGCON);
			cfg |= SWTRGCMD_I80 | TRGMODE_I80;
			writel(cfg, fbdev->regs + S3C_TRIGCON);
			fbdev->draw_req--;
		}
#if defined(CONFIG_FB_S5P_VSYNC_THREAD)
#ifdef SUPPORT_DYNAMIC_FPS
		/*dyn_fps is 0, FPS 30Hz*/
		if (!fbdev->dyn_fps) {
			if (fbdev->vsync_cnt % 2)
				goto inc_vsync;
		}
#else
#ifdef FIMD_FPS_30
...
#endif
#endif
		fbdev->vsync_info.timestamp = ktime_get();
		wake_up_interruptible_all(&fbdev->vsync_info.wait);
#endif
inc_vsync :
		fbdev->vsync_cnt++;
	}
...
#ifdef CONFIG_FB_I80_CLOCK_GATING
#ifdef CONFIG_FB_I80_POWER_GATING
		if (delayed_work_pending(&fbdev[i]->pwr_delayed_work))
			cancel_delayed_work_sync(&fbdev[i]->pwr_delayed_work);
#endif
		if (delayed_work_pending(&fbdev[i]->clk_delayed_work))
			cancel_delayed_work_sync(&fbdev[i]->clk_delayed_work);
#endif
...
#ifdef CONFIG_FB_I80_CLOCK_GATING
		s3cfb_deactivate_req(fbdev[i], 1, GATE_CLK_REQ);
#else
		if (pdata->clk_off)
...
#ifdef CONFIG_FB_S5P_I80_LCD
		s3cfb_win_map_on(fbdev[i], pdata->default_win, 0x00);
		s3cfb_enable_window(fbdev[i], pdata->default_win);
		if(fbdev[i]->draw_req) {
			dev_info(info->dev, "draw req is not zero\n");
			fbdev[i]->draw_req = 0;
		}
#endif
	...
	for (i = 0; i < FIMD_MAX; i++) {
		...
		if (atomic_read(&fbdev[i]->enabled_win) > 0) {
			...
			if (fbdev[i]->regs) {
				...
				BUG();
#ifdef CONFIG_FB_I80_CLOCK_GATING
			s3cfb_deactivate_req(&fbdev[i], 0, GATE_CLK_REQ);
#else
			if (pdata->clk_off)
				pdata->clk_off(pdev, &fbdev[i]->clock);
#endif
...
#define TUNE_REG_NUM	5

struct reg_info_t tune_info[5] = 
{
...
static int print_reg(struct s3cfb_global *fbdev, char *buf, unsigned int start, unsigned int end)
{
	int i, n;
	int cnt, offset;
	int ret = 0;
	...
	do {
		offset = start + (cnt * 4);
		ret += sprintf(buf+ret, "%08x ", readl(fbdev->regs + offset));
		printk("0x%08x ", readl(fbdev->regs + offset));
		cnt++;
		if (cnt%4 == 0) {
			ret += sprintf(buf+ret, "\n");
			printk("\n");
...
static ssize_t fimd_tune_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int i;
	int ret = 0;
	...
	if (fbdev == NULL) {
		ret = sprintf(buf, "LCD status : OFF, Turn LCD On\n");
		printk("LCD status : OFF, Turn LCD On\n");
		goto err;
	}

	for (i = 0; i < TUNE_REG_NUM; i++) {
		printk("%s : \n", tune_info[i].name);
		ret += sprintf(buf+ret, "%s : \n", tune_info[i].name);
		ret += print_reg(fbdev, buf+ret, tune_info[i].start, tune_info[i].end);
	}

err:
	return ret;
}
static int fimd_tune_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t len)
{
	int i, j;
...
static DEVICE_ATTR(fimd_tune, 0666, fimd_tune_show, fimd_tune_store);
#endif
...
static int fps_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t len)
{
	int rc;
	...
	if (value != fbdev->dyn_fps) {
		if (value) {
			dev_info(dev, "set dynamic fps to 60hz\n");
			fbdev->dyn_fps = 1;

		} else {
			dev_info(dev, "set dynamic fps to 30hz\n");
			fbdev->dyn_fps = 0;
			fbdev->vsync_cnt = 1;
		}
	}
...
static DEVICE_ATTR(fps, 0220, NULL, fps_store);
#endif

#ifdef CONFIG_FB_I80_CLOCK_GATING
static void clk_gate_handler(struct work_struct *data)
{
	struct s3cfb_global *fbdev;
	struct s3c_platform_fb *pdata;
	struct platform_device *pdev;

	fbdev = (struct s3cfb_global *)container_of(data,
				struct s3cfb_global, clk_delayed_work.work);
	pdev = to_platform_device(fbdev->dev);
	pdata = to_fb_plat(fbdev->dev);

	if (fbdev->system_state != POWER_ON) {
		printk("%s system power off\n", __func__);
		return;
	}
	printk("%s was called\n", __func__);
	
	if (fbdev->fb_act_req == FB_ACTIVATE_REQ) {
		printk("%s cancel deactivate\n", __func__);
		fbdev->clk_stat = FB_CLK_ON;
		return;
	}
	s3cfb_deactivate_req(fbdev, 1, GATE_CLK_REQ);

#ifdef CONFIG_FB_I80_POWER_GATING
	if (fbdev->fb_act_req == FB_DEACTIVATE_REQ)
		queue_delayed_work(fbdev->pwr_workqueue,
			&fbdev->pwr_delayed_work, msecs_to_jiffies(150));
#endif
}

#ifdef CONFIG_FB_I80_POWER_GATING
static void pwr_gate_handler(struct work_struct *data)
{
	struct s3cfb_global *fbdev;
	struct s3c_platform_fb *pdata;
	struct platform_device *pdev;
	...
	fbdev = (struct s3cfb_global *)container_of(data,
				struct s3cfb_global, pwr_delayed_work.work);
	pdev = to_platform_device(fbdev->dev);
	pdata = to_fb_plat(fbdev->dev);
	...
	s3cfb_deactivate_req(fbdev, 1, GATE_PWR_REQ);
...
#endif
#ifdef CONFIG_FB_I80_CLOCK_GATING
		fbdev[i]->fb_act_req = FB_DEACTIVATE_REQ;
		fbdev[i]->clk_stat = FB_CLK_OFF;
		mutex_init(&fbdev[i]->gate_mutex);
		s3cfb_activate_req(fbdev[i], 0);
#else
		if (pdata->clk_on)
			pdata->clk_on(pdev, &fbdev[i]->clock);
...
#ifdef CONFIG_FB_S5P_I80_LCD
		if (fbdev[i]->lcd->te_irq) {
			irq_set_status_flags(fbdev[i]->lcd->te_irq, IRQ_NOAUTOEN);	

			if (request_irq(fbdev[i]->lcd->te_irq, s3cfb_irq_te,
					IRQF_DISABLED | IRQF_TRIGGER_RISING, "te", fbdev[i])) {
...
#ifdef CONFIG_FB_I80_CLOCK_GATING
		fbdev[i]->clk_workqueue = create_singlethread_workqueue("clock_gating");
		if (!fbdev[i]->clk_workqueue) {
			dev_err(fbdev[i]->dev, "failed to create workqueue for clock gating\n");
			ret = -ENOMEM;
			goto err2;
		}
		INIT_DELAYED_WORK(&fbdev[i]->clk_delayed_work, clk_gate_handler);
#ifdef CONFIG_FB_I80_POWER_GATING
		fbdev[i]->pwr_workqueue = create_singlethread_workqueue("power-gating");
		if (!fbdev[i]->pwr_workqueue) {
			dev_err(fbdev[i]->dev, "failed to create workqueue for power gating\n");
			ret = -ENOMEM;
			goto err2;
		}
		INIT_DELAYED_WORK(&fbdev[i]->pwr_delayed_work, pwr_gate_handler);
#endif
#endif
...
/*
#ifdef FIMD_OVE_TUNE
		ret = device_create_file(fbdev[i]->dev, &dev_attr_fimd_tune);
		if (ret < 0)
			dev_err(fbdev[0]->dev, "failed to add sysfs entries\n");
#endif
*/
...
#ifdef SUPPORT_DYNAMIC_FPS
		ret = device_create_file(fbdev[i]->dev, &dev_attr_fps);
		if (ret < 0)
			dev_err(fbdev[0]->dev, "failed to add sysfs entries\n");
#endif
...
#ifdef CONFIG_FB_S5P_I80_LCD
		free_irq(fbdev[i]->lcd->te_irq, fbdev[i]); 
#endif
		free_irq(fbdev[i]->irq, fbdev[i]);
		iounmap(fbdev[i]->regs);
#ifdef CONFIG_FB_I80_CLOCK_GATING
