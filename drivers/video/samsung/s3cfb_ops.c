...
int fb_is_primary_device(struct fb_info *fb)
{
	...
	dev_dbg(fbdev->dev, "[fb%d] win%d: checking for primary device\n", fb->node, win->id);
...
extern int s5p_dsim_activate(int req_type);
extern int s5p_dsim_deactivate(int req_type);

static int s3cfb_activate_clk(struct s3cfb_global *fbdev, int force)
{
	int ret = 0;
	struct s3c_platform_fb *pdata = to_fb_plat(fbdev->dev);
	struct platform_device *pdev = to_platform_device(fbdev->dev);

	if (fbdev->clk_stat == FB_CLK_ON)
		return ret;

	s5p_dsim_activate(GATE_CLK_REQ);

	if (pdata->clk_on) {
		printk("fimd clk on\n");
		ret = pdata->clk_on(pdev, &fbdev->clock);
	}
...
static int s3cfb_deactivate_clk(struct s3cfb_global *fbdev, int force)
{
	int ret = 0;
	struct s3c_platform_fb *pdata = to_fb_plat(fbdev->dev);
	struct platform_device *pdev = to_platform_device(fbdev->dev);

	if(fbdev->clk_stat == FB_CLK_OFF)
		return ret;

	if (pdata->clk_off) {
		printk("fimd clk off\n");
		ret = pdata->clk_off(pdev, &fbdev->clock);
	}
	s5p_dsim_deactivate(GATE_CLK_REQ);
	
	fbdev->clk_stat = FB_CLK_OFF;
...
static int s3cfb_activate_pwr(struct s3cfb_global *fbdev, int force)
{
	int j;
	int ret = 0;
	struct fb_info *fb;
	...
	s3cfb_win_map_on(fbdev, pdata->default_win, 0x00);
	s3cfb_enable_window(fbdev, pdata->default_win);
	if(fbdev->draw_req) {
		dev_info(fbdev->dev, "draw req is not zero\n");
		fbdev->draw_req = 0;
	}
#endif
...
static int s3cfb_deactivate_pwr(struct s3cfb_global *fbdev, int force)
{
	int ret = 0;
	...
	if (fbdev->clk_stat == FB_PWR_OFF)
		return ret;

	printk("fimd power off\n");

	s5p_dsim_deactivate(GATE_PWR_REQ);
...
#ifdef CONFIG_FB_S5P_SYSMMU
	if (fbdev->sysmmu.enabled == true) {
		fbdev->sysmmu.enabled = false;
		fbdev->sysmmu.pgd = 0;
		s5p_sysmmu_disable(fbdev->dev);
	}
#endif	
	...
	fbdev->clk_stat = FB_PWR_OFF;

	return ret;
}
#endif
int s3cfb_activate_req(struct s3cfb_global *fbdev, int force)
{
	int ret = 0;
	
	fbdev->fb_act_req = FB_ACTIVATE_REQ;

	if (fbdev->clk_stat == FB_CLK_WAIT_OFF) {
		if (delayed_work_pending(&fbdev->clk_delayed_work)) {
			cancel_delayed_work_sync(&fbdev->clk_delayed_work);
		fbdev->clk_stat = FB_CLK_ON;
		}
	}

#ifdef CONFIG_FB_I80_POWER_GATING
	if (delayed_work_pending(&fbdev->pwr_delayed_work)) {
		cancel_delayed_work_sync(&fbdev->pwr_delayed_work);
		printk("pwr_gate_handler canceled\n");
	}
#endif

	mutex_lock(&fbdev->gate_mutex);
	if (fbdev->clk_stat == FB_CLK_OFF)
		s3cfb_activate_clk(fbdev, force);
#ifdef CONFIG_FB_I80_POWER_GATING
	if (fbdev->clk_stat == FB_PWR_OFF)
		s3cfb_activate_pwr(fbdev, force);
#endif
exit:

	mutex_unlock(&fbdev->gate_mutex);
	return ret;
}

int s3cfb_deactivate_req(struct s3cfb_global *fbdev, int force, int req)
{
	int ret = 0;

	fbdev->fb_act_req = FB_DEACTIVATE_REQ;
	...
	if (force) {
		if (req == GATE_CLK_REQ)
			s3cfb_deactivate_clk(fbdev, force);
#ifdef CONFIG_FB_I80_POWER_GATING
		else
			s3cfb_deactivate_pwr(fbdev, force);
#endif
		goto exit;
	}
	
	fbdev->clk_stat = FB_CLK_WAIT_OFF;
	queue_delayed_work(fbdev->clk_workqueue,
		&fbdev->clk_delayed_work, msecs_to_jiffies(50));
...
#endif
#if defined(CONFIG_CPU_EXYNOS4212) || defined(CONFIG_CPU_EXYNOS4412)
#if defined (CONFIG_BUSFREQ_OPP) && !defined(CONFIG_MACH_WATCH) 
...
int s3cfb_map_video_memory(struct s3cfb_global *fbdev, struct fb_info *fb)
{
	...
	dev_info(fbdev->dev, "[fb%d] win%d: Alloc dma: 0x%08x, "
			"size: 0x%08x\n", fb->node, win->id,
			...
			struct fb_var_screeninfo *var, struct fb_info *fb)
{
	...
	dev_dbg(fbdev->dev, "[fb%d] win%d: check_var\n", fb->node, win->id);
...
int s3cfb_blank(int blank_mode, struct fb_info *fb)
{
	...
	case FB_BLANK_UNBLANK:
		if (!fb->fix.smem_start) {
			dev_info(fbdev->dev, "[fb%d] win%d: no allocated memory \
				for unblank\n", fb->node, win->id);
			break;
		}

		if (win->power_state == FB_BLANK_UNBLANK) {
			dev_info(fbdev->dev, "[fb%d] win%d: already in \
				FB_BLANK_UNBLANK\n", fb->node, win->id);
			break;
		} else {
		...
		if (win->enabled) {	/* from FB_BLANK_NORMAL */
			...
			s3cfb_enable_window(fbdev, win->id);
			if (!s3cfb_get_win_cur_buf_addr(fbdev, win->id))
	...
	case FB_BLANK_NORMAL:
		if (win->power_state == FB_BLANK_NORMAL) {
			dev_info(fbdev->dev, "[fb%d] win%d: already in FB_BLANK_NORMAL\n", fb->node, win->id);
			break;
		} else {
			s3cfb_update_power_state(fbdev, win->id,
						FB_BLANK_NORMAL);
	...
	case FB_BLANK_POWERDOWN:
		if (win->power_state == FB_BLANK_POWERDOWN) {
			dev_info(fbdev->dev, "[fb%d] win%d: already in FB_BLANK_POWERDOWN\n", fb->node, win->id);
			break;
		} else {
			s3cfb_update_power_state(fbdev, win->id,
						FB_BLANK_POWERDOWN);
...
#ifdef CONFIG_FB_SUPPORT_ALPM
		if (get_alpm_mode()) {
			dev_info(fbdev->dev, "alpm buf cnt : %d\n", fbdev->alpm_buf_cnt);
			s3cfb_window_on(fbdev, ALPM_WIN_NUM);
			fbdev->alpm_buf_cnt = 0;
		}
#endif
		s3cfb_disable_window(fbdev, win->id);
		s3cfb_win_map_off(fbdev, win->id);
...
void s3c_fb_update_regs(struct s3cfb_global *fbdev, struct s3c_reg_data *regs)
{
	...
	for (i = 0; i < pdata->nr_wins; i++) {
		...
		writel(regs->win_rgborder[i], fbdev->regs + WIN_RGB_ORDER(i));
		...
		writel(regs->vidw_alpha0[i],
				fbdev->regs + S3C_VIDW_ALPHA0(i));
		writel(regs->vidw_alpha1[i],
				fbdev->regs + S3C_VIDW_ALPHA1(i));
	...
	do {
#if defined(CONFIG_FB_S5P_VSYNC_THREAD)
		ret = s3cfb_wait_for_vsync(fbdev, HZ/10);
		if (ret && !fbdev->system_state) {
			dev_err(fbdev->dev, "%s:system_state off\n", __func__);
			break;
		}
#else
		...
		struct s3c_reg_data *regs)
{
	...
	if (win_config->blending >= S3C_FB_BLENDING_MAX) {
		dev_err(fbdev->dev, "unknown blending %u\n",
				win_config->blending);
		return -EINVAL;
	}

	if (win_no == 0 && win_config->blending != S3C_FB_BLENDING_NONE) {
		dev_err(fbdev->dev, "blending not allowed on window 0\n");
		return -EINVAL;
	}
	...
	regs->vidw_alpha0[win_no] = vidw_alpha((win_no == 0 ? 0:1),
			alpha0, alpha0, alpha0);
	regs->vidw_alpha1[win_no] = vidw_alpha((win_no == 0 ? 0:1),
			alpha1, alpha1, alpha1);
...
check_panel_status:
	if (!get_panel_status()) {
		...
		dev_err(fbdev->dev, "wating panel status\n");
		goto check_panel_status;
	}
...
int s3cfb_open_fifo(int id, int ch, int (*do_priv) (void *), void *param)
{
	...
	dev_dbg(fbdev->dev, "[fb%d] win%d: open fifo\n", fb->node, win->id);
...
int s3cfb_close_fifo(int id, int (*do_priv) (void *), void *param)
{
	...
	dev_dbg(fbdev->dev, "[fb%d] win%d: close fifo\n", fb->node, win->id);
