	...
	struct dsim_lcd_config *main_lcd, struct dsim_lcd_config *sub_lcd)
{
	...
	if (main_lcd->e_interface == DSIM_VIDEO) {
		printk("DSIM VIDEO MODE.. \n");
		reg |= (1 << 25);
	
	}
	else if (main_lcd->e_interface == DSIM_COMMAND) {
		printk("DSIM COMMAND MODE.. \n");
		reg &= ~(1 << 25);
...
unsigned int s5p_dsim_get_stop_state_clock(unsigned int dsim_base)
{
	unsigned int reg;
	
	reg = readl(dsim_base + S5P_DSIM_STATUS) & (0x01 << 8);
