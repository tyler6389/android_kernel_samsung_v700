...
void report_proximity_data(struct ssp_data *data, struct sensor_value *prox)
{
	ssp_dbg("[SSP] Proximity Sensor Detect : %u, raw : %u\n",
		prox->prox[0], prox->prox[1]);

	data->buf[PROXIMITY_SENSOR].prox[0] = prox->prox[0];
	data->buf[PROXIMITY_SENSOR].prox[1] = prox->prox[1];
...
void report_proximity_raw_data(struct ssp_data *data,
	struct sensor_value *prox)
{
	if (data->uFactoryProxAvg[0]++ >= PROX_AVG_READ_NUM) {
		...
		data->uFactoryProxAvg[2] += prox->prox[0];

		if (data->uFactoryProxAvg[0] == 1)
			data->uFactoryProxAvg[1] = prox->prox[0];
		else if (prox->prox[0] < data->uFactoryProxAvg[1])
			data->uFactoryProxAvg[1] = prox->prox[0];

		if (prox->prox[0] > data->uFactoryProxAvg[3])
			data->uFactoryProxAvg[3] = prox->prox[0];
	}

	data->buf[PROXIMITY_RAW].prox[0] = prox->prox[0];
}

void get_proximity_sensordata(char *pchRcvDataFrame, int *iDataIdx,
...
void get_proximity_rawdata(char *pchRcvDataFrame, int *iDataIdx,
	struct sensor_value *sensorsdata)
{
	sensorsdata->prox[0] = (u8)pchRcvDataFrame[(*iDataIdx)++];
}
...
static unsigned char read_proximity_rawdata(struct ssp_data *data)
{
...
static int proximity_store_cancelation(struct ssp_data *data, int iCalCMD)
{
	...
	if (iCalCMD) {
		data->uProxCanc = read_proximity_rawdata(data);
		get_proximity_threshold(data);
	} else {
		data->uProxHiThresh = data->uProxHiThresh_default;
		data->uProxLoThresh = data->uProxLoThresh_default;
...
int initialize_proximity_sensor(struct ssp_data *data)
{
	int iRet;
	...
	iRet = device_create_file(&input_dev->dev, &dev_attr_prox_poll_delay);
	if (iRet < 0)
		goto err_creat_file;

	iRet = sensors_register(data->prox_device, data, prox_attrs, MODULE_NAME);
	if (iRet < 0)
		goto err_sensor_register;

	data->prox_input_dev = input_dev;
	return SUCCESS;

err_sensor_register:
	device_remove_file(&input_dev->dev, &dev_attr_prox_poll_delay);
err_creat_file:
...
void remove_proximity_sensor(struct ssp_data *data)
{
