...
static int check_accel_rawdata(struct ssp_data *data)
{
	if ((data->buf[ACCELEROMETER_SENSOR].x == 0)
		&& (data->buf[ACCELEROMETER_SENSOR].y == 0)
		&& (data->buf[ACCELEROMETER_SENSOR].z == 0))
		return FAIL;
	...
	struct device_attribute *attr, char *buf)
{
	...
	while (iRetries++ < 50) {
		if (check_accel_rawdata(data) == SUCCESS)
			break;
		msleep(10);
	}

	if (iRetries >= 50)
		pr_err("[SSP]: %s - accel_raw data is 0!\n", __func__);
