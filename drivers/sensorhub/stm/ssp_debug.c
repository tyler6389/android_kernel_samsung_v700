...
#define LIMIT_INSTRUCTION_FAIL_CNT	3
#define LIMIT_IRQ_FAIL_CNT		2
#define LIMIT_TIMEOUT_CNT		5
#define LIMIT_SPI_FAIL_CNT		3

#define	SSP_BYPASS_SENSORS_EN_ALL	(1 << ACCELEROMETER_SENSOR |\
	1 << GYROSCOPE_SENSOR | 1 << MAGNETIC_SENSOR | 1 << BAROMETER_SENSOR |\
...
static void print_sensordata(struct ssp_data *data, unsigned int uSensor)
{
	...
	case BAROMETER_SENSOR:
		ssp_dbg(" %u : %d, %d (%ums)\n", uSensor,
			data->buf[uSensor].barometer[0],
			data->buf[uSensor].barometer[1],
			get_msdelay(data->adDelayBuf[uSensor]));
		break;
	case GESTURE_SENSOR:
...
static void reset_work_func(struct work_struct *work)
{
	...
	mutex_lock(&data->reset_mutex);
	func_dbg();

	ssp_enable(data, false);

	if (startup_mcu(data) < 0) {
		mutex_unlock(&data->reset_mutex);
	...
	if (data->bSspSleep == true)
		ssp_send_cmd(data, MSG2SSP_AP_STATUS_SLEEP);

	mutex_unlock(&data->reset_mutex);
	wake_lock_timeout(&data->ssp_wake_lock, 3 * HZ);
}

int ssp_reset_noti(struct ssp_data *data)
{
	ssp_dbg("[SSP]: %s(%d)\n", __func__, data->bSspSleep);
	data->uResetNotiCnt++;
...
static void debug_work_func(struct work_struct *work)
{
	...
	ssp_dbg("[SSP]: %s(%u) - Sensor state: 0x%x, RC: %u, MS: %u, RN: %u\n",
		__func__, data->uIrqCnt, data->uSensorState, data->uResetCnt,
		data->uMissSensorCnt, data->uResetNotiCnt);
...
int initialize_debug_timer(struct ssp_data *data)
{
	...
	INIT_WORK(&data->work_reset, reset_work_func);
