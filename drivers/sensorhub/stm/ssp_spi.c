...
int waiting_wakeup_mcu(struct ssp_data *data)
{
	...
	mutex_unlock(&data->wakeup_mutex);
...
static int ssp_spi_sync(struct ssp_data *data, u8 *out_buf,
...
static int ssp_spi_readProc(struct ssp_data *data, char *pTxData, u16 uTxLength,
	char *pRxData, u16 uRxLength)
{
	...
	iRet = ssp_spi_sync(data, pSynctxbuf, uRxsize, pSyncrxbuf);
	if (iRet < 0)
		pr_err("[SSP]: ssp_spi_sync error %d\n", iRet);

	nStartpos = ssp_spi_checkrecvstart(pSyncrxbuf, uRxsize);

	memset(pSynctxbuf, 0, uTxLength);
	while(nStartpos < 0 && nRetry < 100) {
		do_gettimeofday(&cur_time);
		iDiffTime = (int)cur_time.tv_sec - iTimeTemp;
		if ((iDiffTime >= 4) && (data->bSspSleep == false)) {
			pr_err("[SSP]: %s - spi timeout %dsec, retry(%d)\n",
				__func__, iDiffTime, nRetry);
			kfree(pSyncrxbuf);
			kfree(pSynctxbuf);
			return ERROR;
		}

		ssp_spi_sync(data, pSynctxbuf, uRxsize, pSyncrxbuf);
		nStartpos = ssp_spi_checkrecvstart(pSyncrxbuf, uRxsize);
		nRetry++;
	}
	...
	char *pRxData, u16 uRxLength, int iRetries)
{
	...
	do {
		iRet = ssp_spi_readProc(data, pTxData, uTxLength,
			pRxData, uRxLength);
		if (iRet != FAIL)
			break;
		pr_err("[SSP]: %s - retry.. %d\n", __func__, iRetries);
		msleep(100);
	} while (--iRetries >= 0);
