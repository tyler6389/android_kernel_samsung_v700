...
#define MSG2AP_INST_RESET_NOTI_DATA		0xAF
...
void ssp_enable(struct ssp_data *data, bool enable)
{
	...
	pr_info("%s, enable irq = %d, bSspShutdown = %d\n",
	...
	if (enable && data->bSspShutdown) {
		...
		pr_err("%s, error / enable irq = %d, bSspShutdown = %d\n",
			__func__, enable, data->bSspShutdown);
	...
	unsigned int uChangedSensor, unsigned int uNewEnable)
{
	...
	memset(&data->buf[uChangedSensor], 0, sizeof(struct sensor_value));
	return 0;
}
	...
	for (iDataIdx = 0; iDataIdx < iLength;) {
		if (pchRcvDataFrame[iDataIdx] == MSG2AP_INST_BYPASS_DATA) {
			...
			break;
					...
					MSG2AP_INST_RESET_NOTI_DATA) {
				pr_info("[SSP]: %s - Mcu reset noti!!\n",
					__func__);
				ssp_reset_noti(data);
			break;
		} else
