...
static struct io_device *get_real_iod(struct io_device *iod,
				struct link_device *ld, struct sk_buff *skb)
{
	struct header_data *hdr = &fragdata(iod, ld)->h_data;
	struct sipc5_link_hdr *sipc5h = (hdr->frag_len) ?
		(struct sipc5_link_hdr *)hdr->hdr :
		(struct sipc5_link_hdr *)skb->data;
	return (link_get_iod_with_channel(ld, sipc5h->ch)) ?: iod;
}
				...
				struct link_device *ld,	struct sk_buff *skb_in)
{
	...
	if (hdr->frag_len < hdr_len) {
		...
		real_iod = get_real_iod(real_iod, ld, skb);
		/* Send SIPC5 Header skb to next stage*/
		ret = real_iod->ops.recv_skb_packet(real_iod, ld, skb);
		if (unlikely(ret < 0)) {
